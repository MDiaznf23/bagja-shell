...
get_cover() {
    ...
    local default_cover="$HOME/.config/quickshell/assets/default-cover.jpg"
...
build_json() {
    ...
    # pastikan position dan length adalah angka
...
if [ -n "$SELECTED" ]; then
    playerctl metadata -p "$SELECTED" --format "$FORMAT" -F 2>/dev/null | \
    while IFS="$SEP" read -r _ player status title artist art_url file_path position length _; do
        build_json "$player" "$status" "$title" "$artist" "$art_url" "$file_path" "$position" "$length"
    done
else
    playerctl metadata --format "$FORMAT" -F 2>/dev/null | \
    while IFS="$SEP" read -r _ player status title artist art_url file_path position length _; do
        build_json "$player" "$status" "$title" "$artist" "$art_url" "$file_path" "$position" "$length"
    done
fi
